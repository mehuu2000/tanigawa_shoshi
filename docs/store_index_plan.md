...
`all_tokens` は候補検索用の統合トークン集合 `C = ⋃_{f ∈ F} C_f` として使う。
...
`all_tokens` は `authors`, `title`, `journal`, `year`, `volume`, `page` から生成したトークンを統合した `C = ⋃_{f ∈ F} C_f` に対応する。
...
👉 **フィールド単位トークン + 統合集合検索（C = ⋃_{f ∈ F} C_f）**
