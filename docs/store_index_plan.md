...
| フィールド          | stored | indexed | docValues |
| -------------- | ------ | ------- | --------- |
| authors_tokens | true  | false    | false |
| first_author_tokens | true | false | false |
| title_tokens   | true  | false    | false |
| journal_tokens | true  | false    | false |
| year_tokens    | true  | false    | false |
| volume_tokens  | true  | false    | false |
| page_tokens    | true  | false    | false |

フィールド別トークンは後続フェーズの RC / CC / MC 計算で使うため、`stored=true`, `indexed=false`, `docValues=false` とする。
...
`first_author_tokens` は CC 計算用なので、`all_tokens` には含めない。
...
除外時は `log/` 配下の時刻付きログファイルへ、tokenize 後に必須 token field が空になった文書のみ MongoDB `_id`・対象フィールド・元の値・理由を記録する。
...
* フィールド単位のトークン集合 Cf は stored field として保持
...
`all_tokens` は `authors_tokens`, `title_tokens`, `journal_tokens`, `year_tokens`, `volume_tokens`, `page_tokens` を統合した `C = ⋃Cf` に対応する。
`first_author_tokens` は検索対象には含めず、後続の CC 計算で使用する。
...
* volume は MongoDBの volume と issue を同じ配列内に分けて保持し、volume_tokens も同じフィールドにまとめる
